...
from mapf_transformer.model import AgentInteractionEncoder, MAPFTransformer
...
def test_agent_local_encoder_and_interaction_latents_make_265_token_context():
    ...
    frames, _, token_valid, attention_allowed = model.encode_frames(batch)
    ...
    assert attention_allowed is None
...
def test_distance_graph_masks_far_agents_and_runs_forward_backward():
    config = ModelConfig(
        d_model=32,
        n_heads=4,
        temporal_layers=1,
    ...
    encoder = AgentInteractionEncoder(config)
    x = torch.zeros((1, 25), dtype=torch.long)
    y = torch.zeros((1, 25), dtype=torch.long)
    x[0, :3] = torch.tensor([0, 3, 4])
    valid = torch.zeros((1, 25), dtype=torch.bool)
    valid[0, :3] = True
    allowed = encoder._build_attention_allowed(
        x, y, valid, torch.ones((1, 0), dtype=torch.bool)
    )
    assert allowed is not None
    assert allowed[0, 0, 1]
    assert not allowed[0, 0, 2]

    episode = generate_synthetic_episode(seed=13, num_agents=2, max_steps=4)
    sample = SequenceSampleBuilder(config).build(episode, ego_id=0, time_step=0)
    batch = {key: value.unsqueeze(0) for key, value in sample.items()}
    model = MAPFTransformer(config)
    frames, _, _, frame_allowed = model.encode_frames(batch)
    assert frames.shape[1:3] == (8, 26)
    assert frame_allowed is not None
    assert frame_allowed.shape == (1, 8, 26, 26)
    output = model(batch)
    assert torch.isfinite(output.loss)
    output.loss.backward()
    assert model.agent_set_encoder.blocks[0].attn.in_proj_weight.grad is not None
