...
def main() -> None:
    ...
    axes[0, 0].set(title="Training and validation loss", xlabel="Optimizer step", ylabel="Cross-entropy loss")
    axes[0, 0].legend()

    if len(validation):
        axes[0, 1].plot(val_steps, val_accuracy * 100, "o-", color="#54A24B", markersize=4)
        best_index = int(np.argmax(val_accuracy))
        axes[0, 1].scatter(val_steps[best_index], val_accuracy[best_index] * 100, color="#1B6E1B", zorder=3)
        axes[0, 1].annotate(
    ...
    axes[0, 1].set(title="Validation action accuracy", xlabel="Optimizer step", ylabel="Accuracy (%)")

    axes[1, 0].plot(train_steps, learning_rate, color="#B279A2", linewidth=2)
    ...
    axes[1, 1].plot(train_steps, throughput, color="#F58518", alpha=0.3, linewidth=0.8)
    axes[1, 1].plot(train_steps, moving_average(throughput, max(1, args.smooth)), color="#B85C00", linewidth=2)
    axes[1, 1].set(title="Training throughput", xlabel="Optimizer step", ylabel="Samples / second")

    final_step = complete.get("step", int(train_steps[-1]))
    final_val_loss = complete.get("validation_loss", "n/a")
    ...
    fig.suptitle(
        ...
        f"final val loss {float(final_val_loss):.4f}, final val accuracy {accuracy_text}",
        fontsize=15,
    )
