...
class EpisodeFeatureGenerator {
 public:
  ...
  py::dict build(int step) {
    ...
    py::array_t<bool> incore({B,N,A}),free({B,N,A}),greedy({B,N,A}),bneck({B,N,A});
    ...
    auto IC=incore.mutable_unchecked<3>();auto FR=free.mutable_unchecked<3>();auto GR=greedy.mutable_unchecked<3>();auto BN=bneck.mutable_unchecked<3>();
    ...
    for(int ego=0;ego<B;++ego){
      ...
      for(int s=0;s<N;++s){
        ...
        for(int ac=0;ac<A;++ac){TG(ego,s,ac,0)=TG(ego,s,ac,1)=0;IC(ego,s,ac)=FR(ego,s,ac)=GR(ego,s,ac)=BN(ego,s,ac)=false;OH(ego,s,ac)=max_hops_+1;DE(ego,s,ac)=5;}
        ...
        for(int ac=0;ac<A;++ac){RC q={p[0]+DR[ac],p[1]+DC[ac]};int rr=p[0]-ep[0]+DR[ac]+center,cc=p[1]-ep[1]+DC[ac]+center;TG(ego,s,ac,0)=rr;TG(ego,s,ac,1)=cc;IC(ego,s,ac)=rr>=0&&rr<core_&&cc>=0&&cc<core_;bool f=inside(q[0],q[1])&&obstacles_[index(q)]==0;FR(ego,s,ac)=f;if(!f){DE(ego,s,ac)=3;continue;}int td=distances_[gid][index(q)];OH(ego,s,ac)=td==INF?-1:td;if(cd==INF||td==INF)DE(ego,s,ac)=4;else if(td<cd){DE(ego,s,ac)=0;GR(ego,s,ac)=true;}else if(td>cd)DE(ego,s,ac)=2;else DE(ego,s,ac)=1;BN(ego,s,ac)=degree_[index(q)]<=2;}if(CH(ego,s)==0)GR(ego,s,0)=true;
      }
 ...
 private:
  ...
  std::vector<int>rank_current(int t,int ego){std::vector<int>v;for(int i=0;i<n_;++i)if(i!=ego&&is_visible(t,ego,i))v.push_back(i);auto ec=candidate(t,ego);RC ep=pos(t,ego);std::sort(v.begin(),v.end(),[&](int a,int b){auto ca=candidate(t,a),cb=candidate(t,b);auto key=[&](int id,const auto&c){bool vertex=overlap(ec.second,c.second)>0,edge=false;for(auto&x:ec.second)for(auto&y:c.second)if(x==pos(t,id)&&y==ep)edge=true;int ov=overlap(ec.first,c.first);bool cor=false;for(auto&x:ec.second)if(c.second.count(x)&&degree_[index(x)]<=2)cor=true;RC p=pos(t,id);int md=std::abs(p[0]-ep[0])+std::abs(p[1]-ep[1]);return std::tuple<int,int,int,int,int>{-(vertex||edge),-ov,-cor,md,id};};return key(a,ca)<key(b,cb);});v.insert(v.begin(),ego);if((int)v.size()>max_current_)v.resize(max_current_);return v;}
  std::vector<int>history_ids(int t,int ego,const std::vector<int>&current){std::vector<std::pair<double,int>>s;for(int gid:current)if(gid!=ego){double score=0;for(int lag=0;lag<=history_;++lag){int tau=t-lag;if(tau<0||!is_visible(tau,ego,gid))continue;score+=2.;RC p=pos(tau,gid),e=pos(tau,ego);score+=1./(1.+std::abs(p[0]-e[0])+std::abs(p[1]-e[1]));auto a=candidate(tau,ego),b=candidate(tau,gid);score+=2.*(overlap(a.second,b.second)>0)+.25*overlap(a.first,b.first);}s.push_back({-score,gid});}std::sort(s.begin(),s.end());std::vector<int>out{ego};for(int i=0;i<(int)s.size()&&i<tracks_-1;++i)out.push_back(s[i].second);return out;}
